...
public class ControllerTest {
	...
	public void whenGetAllTodo_ThenWillReturnJson() throws Exception{
		...
		.andExpect(jsonPath("$[0].title", equalTo("todo test1")));
	...
	public void whenPostTodo_ThenCreateTodo() throws Exception{
		...
		Todo todo1 = new Todo("1", "todo test1", "test 1");		
		BDDMockito.given(todoser.addTodo(Mockito.any(Todo.class))).willReturn(todo1);
		
		// to convert object to json 
		ObjectMapper mapper = new ObjectMapper();
		moc.perform(post("/api/v1/todo/").contentType(MediaType.APPLICATION_JSON).
				content(mapper.writeValueAsString(todo1)))
		.andExpect(status().isCreated())
		.andExpect(jsonPath("$.title", equalTo("todo test1")));
